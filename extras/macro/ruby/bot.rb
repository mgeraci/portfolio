...
class Bot
  ...
  # Initializes the bot object, loads in the externam YAML data, and sets the bot's name.
  # Raises an exception if the loading process fails.
  ...
  # Using a supplied sentence, go through the bot's :responses data set and collect all possible responses
  def possible_responses(sentence)
    ...
    @data[:responses].keys.each do |pattern|
      ...
      if sentence.match('\b' + pattern.gsub(/\*/, '') + '\b')
        # If the pattern contains substitution placeholder, perform the substitution
        if pattern.include?('*')
          ...
          # Just add the phrases to the array if there are no placeholders
